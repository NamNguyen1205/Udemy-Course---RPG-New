...
public class Inventory_Storage : Inventory_Base
{
    ...
    private bool HasEnoughMaterial(Inventory_Item itemToCraft)
    {
        ...
        {
            if (GetAvailableAmountOf(requireMaterial.itemData) < requireMaterial.stackSize)
                return false;
        }
    ...
    public int GetAvailableAmountOf(ItemDataSO requiedItem)
    {
        int amount = 0;

        foreach (var item in playerInventory.itemList)
        {
            if (item.itemData == requiedItem)
    ...
    {
        var stackabkeItem = StackableInStash(itemToAdd);

        if (stackabkeItem != null)
            stackabkeItem.AddStack();
        else
        {
